...
public partial class admin_EditClass : System.Web.UI.Page
{
    ...
    protected void link_edit(object sender, System.EventArgs e)
    {
        ...
        cmd.Parameters.Add("c_id", SqlDbType.NVarChar).Value = Selected_id.Value;
    ...
    protected void Btn_save_Click(object sender, System.EventArgs e)
    {
        ...
        cmd.Parameters.Add("c_name", SqlDbType.NVarChar).Value =c_name.Text;   
        if (Btn_save.CommandArgument == "add")
        {
       
        }
        else {
            cmd.Parameters.Add("c_id", SqlDbType.VarChar).Value = Selected_id.Value ;
        }
    ...
    protected void btn_del_Click(object sender, System.EventArgs e)
    {
        ...
        cmd = new SqlCommand("update  category set status = 'D' where c_id = '" + Selected_id.Value + "' ", conn);
        cmd.ExecuteNonQuery();
        cmd.Dispose();
        conn.Close();
    ...
    protected void link_delete(object sender, System.EventArgs e)
    {
        ...
        conn.Open();
        cmd = new SqlCommand("update  category set status = 'D' where c_id = '" + obj.CommandArgument  + "' ", conn);
        cmd.ExecuteNonQuery();
        cmd.Dispose();
        conn.Close();
